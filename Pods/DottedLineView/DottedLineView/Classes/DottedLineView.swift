...
public class DottedLineView: UIView {
    ...
    private func configurePath(path: UIBezierPath, rect: CGRect) {
        if horizontal {
            let line = rect.size.width.truncatingRemainder(dividingBy: (lineWidth * 2))

            let center = rect.height * 0.5
            let drawWidth = rect.size.width - line + lineWidth
            ...
            let line = rect.size.height.truncatingRemainder(dividingBy: (lineWidth * 2))

            let drawHeight = rect.size.height - line + lineWidth
    ...
    private func configureRoundPath(path: UIBezierPath, rect: CGRect) {
        if horizontal {
            ...
            let center = rect.height * 0.5
            let drawWidth = rect.size.width - line
            let startPositionX = (rect.size.width - drawWidth) * 0.5 + lineWidth
            ...
            let drawHeight = rect.size.height - line
            let startPositionY = (rect.size.height - drawHeight) * 0.5 + lineWidth
